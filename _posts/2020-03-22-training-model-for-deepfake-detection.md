...
<p float="left">
  <img src="/assets/images/ssd_architecture.jpg" alt="SSD Architecture" width="600"/>
    <figcaption><strong>SSD model architecture. Source: [Single Shot MultiBox Detector (SSD)](https://arxiv.org/pdf/1512.02325.pdf)</strong></figcaption>
</p>
<p>&nbsp;</p>

BRIEFLY, THIS IS HOW SSD WORKS....

The SSD model was run using the TensorFlow backend for Keras and the Python code for doing so was provided in the invaluable GitHub repository [ssd_keras](https://github.com/pierluigiferrari/ssd_keras) by Pierluigi Ferrari. Because neural networks that process images require graphics processing units (GPUs) to run at reasonable speeds, I trained the SSD model on Google Cloud Platform, which allows users to harness the power of a wide variety of computing resources, including GPUs. In R I formatted my final annotation datasets to be compatible with the input required by the SSD model, and loaded my images and annotations into a Google Cloud Storage Bucket. Rather than submitting computing jobs to Google Cloud Platform through the command line - which is how AI Platform requests are typically handled - I decided to use the new [AI Platform Notebooks](https://cloud.google.com/ai-platform-notebooks). These are JupyterLab notebooks instances where a user can easily alter the storage and computing resources employed by the notebook. In my case I edited my notebooks on CPUs and trained my model on GPUs. After spending the $300 free credit offered to all new users of Google Cloud Platform, I paid a little more than $50 out-of-pocket for the storage and computing resources required by this project.
